...
import { Box, Card, CardContent, CardHeader, darken, IconButton, Typography } from "@mui/material";
import AddIcon from '@mui/icons-material/Add';
import RemoveIcon from '@mui/icons-material/Remove';
import RefreshIcon from '@mui/icons-material/Refresh';
import { districts } from "./districtJson";
import { useGesture } from '@use-gesture/react';
import { District, DistrictContextType, useDistrict } from "../../../providers/DistrictContext";
...
const MaharashtraMap: React.FC<MaharashtraMapProps> = ({ onDistrictClick }) => {
    ...
    return (
        ...
        >
            ...
            >
                <IconButton sx={{ backgroundColor: '#013e5e', color: '#fff', borderRadius: '3px', height: '30px' }} onClick={() => setScale((prev) => Math.min(3, prev + 0.1))}>
                    <AddIcon />
                </IconButton>
                <IconButton sx={{ backgroundColor: '#013e5e', color: '#fff', borderRadius: '3px', height: '30px' }} onClick={() => setScale((prev) => Math.max(0.5, prev - 0.1))}>
                    <RemoveIcon />
                </IconButton>
                <IconButton sx={{ backgroundColor: '#013e5e', color: '#fff', borderRadius: '3px', height: '30px' }} onClick={resetHighlight}>
                    <RefreshIcon />
                </IconButton>
            </Box>
                    ...
                    {districts.map((district) => (
                        <g key={district.id}>
                            <path
                                ...
                                stroke="#000"
                                strokeWidth="2"
                                filter={hoveredDistrict === district.name || selectedDistrict?.name === district.name ? "'drop-shadow(0 0 150px black)'" : "none"}
                                opacity={selectedDistrict?.name === district.name ? 1 : 0.5}
                                style={{
